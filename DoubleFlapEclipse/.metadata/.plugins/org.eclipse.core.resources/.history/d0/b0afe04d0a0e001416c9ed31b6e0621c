...
public class ScrollHandler {
	...
	private static BackGround mountain, mountainf, cloud1, cloud1f, cloud2, cloud2f, cloud3, cloud3f,
	...
	public ScrollHandler(GameWorld gameWorld, float yPos) {
		...
		pipe2 = new Pipe(pipe1.getTailX(1) + PIPE_GAP, 0, 22, 60, SCROLL_SPEED, PIPE_SPACE_E,
				yPos);
		pipe3 = new Pipe(pipe2.getTailX(1) + PIPE_GAP, 0, 22, 60, SCROLL_SPEED, PIPE_SPACE_E,
				yPos);
		
		pipeA = new Pipe(-22, 0, 22, 60, SCROLL_SPEED, PIPE_SPACE_E, yPos);
		pipeB = new Pipe(pipeA.getTailX(-1) - PIPE_GAP, 0, 22, 60, SCROLL_SPEED, PIPE_SPACE_E,
				yPos);
		pipeC = new Pipe(pipeB.getTailX(-1) - PIPE_GAP, 0, 22, 60, SCROLL_SPEED, PIPE_SPACE_E,
				yPos);
		...
		mountain = new BackGround(136, (int) yPos - 43, 136, 43, SCROLL_SPEED_BGR + 5);
		...
		mountain2 = new BackGround(272, (int) yPos - 43, 136, 43, SCROLL_SPEED_BGR + 5);
		...
		mountainf = new BackGround(0, (int) yPos - 43, 136, 43, SCROLL_SPEED_BGL + 5);
		...
		mountainf2 = new BackGround(-136, (int) yPos - 43, 136, 43, SCROLL_SPEED_BGL + 5);
	...
	public boolean collidesLeft(Bird bird) {
		...
		return (pipeA.collides(bird) || pipeB.collides(bird) || pipeC.collides(bird));
	}
	
	public boolean collidesBounds(Bird bird) {
		return Intersector.overlaps(bird.getBoundingCircle(), rightBound) || Intersector.overlaps(bird.getBoundingCircle(), leftBound);
	}
	
	public boolean collidesPowerUp(Bird bird) {
		if (powerUp.getXX() < bird.getX() + bird.getWidth()) {
			if (Intersector.overlaps(bird.getBoundingCircle(), powerUp.getBoundingCircle())){
				return true;
			}
			return false;
		}
