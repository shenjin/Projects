...
public class Pipe extends Scrollable {
	...
	// constructor
	public Pipe(float x, float y, int width, int height, float scrollSpeed, float gapSpace,
			float groundY) {
	...
	public void update(float delta, int type) {
		...
		barDown.set(position.x, position.y + height + gapSpace, width,
				groundY - (position.y + height + gapSpace));
	...
	public void reset(float newX, int type) {
		...
		height = r.nextInt(((int) GameWorld.getMidPointY() * 2) - ((int) getGapSpace()) - 33) + 11;
		isScored = false;
	}
