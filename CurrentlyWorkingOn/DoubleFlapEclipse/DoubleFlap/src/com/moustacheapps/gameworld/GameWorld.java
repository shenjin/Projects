...
public class GameWorld {
	...
	public void updateRunning(float delta) {
		...
		if ((bird.isAlive() && scroller.collidesPowerUp(bird)) || bird2.isAlive() && scroller.collidesPowerUp(bird2)){
			lives = lives + 1;
			addScore(5);
		...
		if (bird.isAlive() && (scroller.collides(bird) || scroller.collidesBounds(bird))) {
		...
		if (bird2.isAlive() && (scroller.collidesLeft(bird2) || scroller.collidesBounds(bird2))) {
		...
		if (!bird.decelerated() && Intersector.overlaps(bird.getBoundingCircle(), ground)) {
			...
			if (score > AssetLoader.getHighScore() && bird2.getAccelerationY() == 0) {
				AssetLoader.setHighScore(score);
				currentState = GameState.HIGHSCORE;
			}
		}

		if (!bird2.decelerated() && Intersector.overlaps(bird2.getBoundingCircle(), ground)) {
			...
			if (score > AssetLoader.getHighScore() && bird.getAccelerationY() == 0) {
				AssetLoader.setHighScore(score);
				currentState = GameState.HIGHSCORE;
			}
		}
	...
	public boolean isRunning() {
		return currentState == GameState.STARTE 
				|| currentState == GameState.STARTH || currentState == GameState.STARTI;
	}
