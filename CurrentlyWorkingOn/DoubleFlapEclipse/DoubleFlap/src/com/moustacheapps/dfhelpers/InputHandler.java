...
public class InputHandler implements InputProcessor {
	...
	difficultyButtonsInsaneUnlock, inputSections, retryButtons;

	private SimpleButton playButton, easyButton, hardButton, insaneButton, retryButton, 
	hardLockButton, insaneLockButton, leftTap, rightTap, shuffleButtonOn, shuffleButtonOff, shuffleLockButton;
	...
	public InputHandler(GameWorld myWorld, float scaleFactorX, float scaleFactorY) {
		...
		leftTap = new SimpleButton(0, 0, 136, GameWorld.getMidPointY() * 2, null, null);
		rightTap = new SimpleButton(136, 0, 136, GameWorld.getMidPointY() *2, null, null);
		...
		shuffleButtonOff = new SimpleButton(
				...
				midPointY + 40, 29, 16, AssetLoader.shuffleButtonOffUp, AssetLoader.shuffleButtonOffDown);
		shuffleButtonOn = new SimpleButton(
				272 / 2 - (AssetLoader.shuffleButtonOnUp.getRegionWidth() /2),
				midPointY + 40, 29, 16, AssetLoader.shuffleButtonOnUp, AssetLoader.shuffleButtonOnDown);
		shuffleLockButton = new SimpleButton(
				272 / 2 - (AssetLoader.shuffleButtonLockedUp.getRegionWidth() /2),
				midPointY + 40, 29, 16, AssetLoader.shuffleButtonLockedUp, AssetLoader.shuffleButtonLockedDown);
	...
	public boolean touchDown(int screenX, int screenY, int pointer, int button) {
		...
		if (myWorld.isMenu()) {
			...
			if (myWorld.isShuffle()) {
				if (shuffleState == 1 && shuffleButtonOn.isTouchDown(screenX, screenY)) {
					shuffleState = 0;
				} else if (shuffleState == 0 && shuffleButtonOff.isTouchDown(screenX, screenY)) {
					shuffleState = 1;
				}
	...
	public boolean touchUp(int screenX, int screenY, int pointer, int button) {
		...
		if (myWorld.isMenu()) {
			...
			if (easyButton.isTouchUp(screenX, screenY)) {
				...
				if (shuffleState == 0 && shuffleButtonOff.isTouchUp(screenX, screenY)) {
					shuffleState = 1;
					if (shuffleTap) {
						shuffleTap = false;
					} else {
					...
					return true;
				} else if (shuffleState == 1 && shuffleButtonOn.isTouchUp(screenX, screenY)) {
					shuffleState = 0;
					if (shuffleTap) {
						shuffleTap = false;
					} else {
			...
			if (retryButton.isTouchUp(screenX, screenY)) {
				...
				return true;
			} else if (leftTap.isTouchUp(screenX, screenY) || rightTap.isTouchUp(screenX, screenY)){
				myWorld.restart(state);
				myBird.onClick();
				myBird2.onClick();
				return true;
